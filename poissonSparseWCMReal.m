function [E,varargout] = poissonSparseWCMReal(A,N,varargin)

% E = POISSONSPARSEWCMREAL(A,N) takes the weighted, undirected (n x n) adjacency matrix A and creates N
...
%
% MHE: updated preamble to handle small real valued links.
% any links smaller than 1/C of the strongest link (i.e. max(A(:))/C) are set to zero before
% anything else happens. poissonSparseWCM code then continues as normal, 
% with the exception of additional sixth and seventh output variables [..,..,..,..,..,A_z,L_z]
% which return the thresholded A (A_z) and an index of the links set to zero (L_z)
%
%
...
% functions
%
% Silvia Maggi, Mark Humphries and Mathew Evans
...
% ALTERNATIVE CONVERSION THAT PRESERVES ALL THE LINKS 
% A_int = zeros(n,n);
% ixLinks = find(A>0);
% A_int(ixLinks) = ceil(A(ixLinks)*conversion);

% % Conversion that sets links below max/C link strength to 0, but saves the ID of those links  
% % First find all non-zero links
% ixLinks = find(A>0);
% % Determine threshold link strength and find links weaker than max/C
% C_thresh = max(A(:))/conversion;
% tiny_links = find(A(ixLinks)<(C_thresh));
% % [L_z(:,1),L_z(:,2)] = ind2sub(size(A),ixLinks(tiny_links));
